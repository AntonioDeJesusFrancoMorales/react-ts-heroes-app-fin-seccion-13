...
export const HeroGridCard = ({hero}: Props) => {
  const { isFavorite, toggleFavorites } = use(FavoriteHeroesContext);

  const navigate = useNavigate();

  const handleClick = (idSlug: string) => {
    navigate(`/heroes/${idSlug}`)
  };
  ...
  return (
      <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50">
        <div className="relative h-64">
          ...
          >
            <Heart className={`h-4 w-4 ${isFavorite(hero) ? 'fill-red-500 text-red-500' : 'text-gray-500 fill-gray-500'}`} />
          </Button>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-1">
              {hero.powers.map((power, index) => (
                <div key={power}>
                  {index < 3  && <Badge variant="outline" className="text-xs">
                    {power}
                  </Badge>}
                </div>
              ))}
